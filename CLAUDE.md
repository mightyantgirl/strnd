...
| # | 확인 항목 | 필수 | 구체적 검증 방법 |
|---|-----------|------|-----------------|
| 1 | 빌드 에러 없음 | ✅ | `npx sass` 컴파일 통과 |
| 2 | 헥스 코드 미사용 | ✅ | 새로 작성한 코드에 `#` + 6자리 패턴 없는지 확인 |
| 3 | 토큰 외 spacing 미사용 | ✅ | `8px`, `15px`, `24px` 등 토큰에 없는 값 없는지 확인 |
| 4 | 기존 컴포넌트 재사용 | ✅ | 기존 컴포넌트로 대체 가능한 부분 없는지 확인 |
| 5 | 토큰 추가 시 동기화 | ✅ | `_tokens.scss`와 `theme.js` 모두 수정했는지 확인 |
...
| 디자인 토큰 없이 스타일링 | 디자인 시스템 일관성 파괴 | 항상 `$token` (SCSS 변수) 사용 |
...
## 프로젝트 스택
- HTML + SCSS (퍼블리싱 단계, React는 추후)
- Dart Sass (`@use` 모듈 시스템)
- 폰트: Pretendard (CDN)
- 디자인 토큰: `src/styles/_tokens.scss`, `src/theme.js`
- 컴포넌트 SCSS: `src/styles/components/`
- 브레이크포인트: T(768px), D(1200px)
